...
// 创建一个根据文件路径来解析当前文件的内容，读取文件内容后提取它的依赖关系
function parseDependence(filename) {
  ...
  //   babel的transform能够将我们的原始代码转换成为抽象语法数（ast）

  const ast = transform(rowCode).ast
  // 当我们解析成为抽象语法树后，我们就可以提取当前文件中的依赖了，存在dependences里面
  const dependences = []
  ...
  traverse(ast, {
    // 类型为`ImportDeclaration`的AST节点，其实就是我们的 `import xxx from xxxx`

    ImportDeclaration({ node }) {
      // 获取节点的值，就是我们 import from xxxx 中的地址
